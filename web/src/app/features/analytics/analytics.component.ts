  ...
  template: `
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div>
        ...
        <p class="ff-page-subtitle">Selecione um formulário para visualizar os dados analíticos</p>
      </div>
    </div>
        ...
        @for (form of filtered(); track form.id) {
          <div class="ff-card p-4 flex items-center gap-4 hover:shadow-card-hover transition-shadow duration-200">
            <div class="w-10 h-10 rounded-lg bg-primary-50 dark:bg-primary-900/40 flex items-center justify-center shrink-0">
              <i class="pi pi-chart-bar text-primary-600 dark:text-primary-300"></i>
            </div>

            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium text-surface-900 dark:text-surface-0 truncate">{{ form.title }}</p>
              <p class="text-xs text-surface-400 mt-0.5">
                Publicado em {{ (form.publishedAt ?? form.updatedAt) | date:'dd/MM/yyyy' }}
              </p>
            </div>
...
export class AnalyticsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.list(0, 200).subscribe({
      next: page => {
        this.forms.set(page.content.filter(f => f.status === 'PUBLISHED'));
  ...
  goToAnalytics(formId: string): void {
    this.router.navigate(['/forms', formId, 'analytics']);
  }
}
