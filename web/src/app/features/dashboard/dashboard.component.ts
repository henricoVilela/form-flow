...
@Component({
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
      @for (stat of stats(); track stat.label) {
        <div class="ff-card flex items-center gap-4">
          ...
          <div>
            <p class="text-2xl font-display font-bold text-surface-900">{{ stat.value }}</p>
    ...
    <div class="mb-6">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-display font-semibold text-surface-900">Formulários recentes</h2>
      ...
      @if (loading()) {
        ...
        <div class="ff-card text-center py-12">
          <div class="w-16 h-16 mx-auto mb-4 bg-surface-50 rounded-2xl
                      flex items-center justify-center">
            <i class="pi pi-file-edit text-2xl text-surface-300"></i>
          </div>
          <h3 class="text-base font-semibold text-surface-700 mb-1">Nenhum formulário ainda</h3>
          ...
          @for (form of recentForms(); track form.id) {
            <div class="ff-card group cursor-pointer" [routerLink]="['/forms', form.id, 'edit']">
              <div class="flex items-start justify-between mb-3">
                <h3 class="text-sm font-semibold text-surface-900 group-hover:text-primary-600
              ...
              <div class="flex items-center gap-4 text-xs text-surface-400 mt-auto pt-3 border-t border-surface-100">
...
export class DashboardComponent implements OnInit {
  ...
  readonly stats = signal([
    { label: 'Formulários',     value: '—', icon: 'pi pi-file-edit',  bgClass: 'bg-blue-50',    iconClass: 'text-blue-600' },
    { label: 'Publicados',      value: '—', icon: 'pi pi-check-circle', bgClass: 'bg-emerald-50', iconClass: 'text-emerald-600' },
    { label: 'Respostas total', value: '—', icon: 'pi pi-inbox',      bgClass: 'bg-violet-50',  iconClass: 'text-violet-600' },
    { label: 'Esta semana',     value: '—', icon: 'pi pi-chart-line', bgClass: 'bg-amber-50',   iconClass: 'text-amber-600' },
  ]);
